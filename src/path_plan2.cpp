...
int main(int argc, char** argv )
{
  ...
  while(ros::ok())
  {
    // while((F_move_cmd[0][0] == 0) && (F_move_cmd[0][1] == 0) && (F_move_cmd[1][0] == 0) && (F_move_cmd[1][1] == 0))
    // {
    //   ROS_INFO("Asleep");
    //   ros::spin();
    //   //r.sleep();
    ...
    // } 
    // ROS_INFO("Woke up");
    // geometry_msgs::Twist g_move;

    // for (int i = 0; i < 2; i++)
    // {
    //   f_cmd_vel[i][0] = F_move_cmd[i][0];
    //   f_cmd_vel[i][1] = F_move_cmd[i][1];
    // }

    // f_goal = D_yaw + f_cmd_vel[0][0];
    ...
    //   {
    //     ros::spinOnce();
    //     g_move.angular.z = -0.26;
    //     Cmd_vel.publish(g_move);
    //     if (f_goal > D_yaw)
    //     {
    //       g_move.angular.z = 0;
    //       Cmd_vel.publish(g_move);
    ...
    // // f_pre_pose[0] = F_cur_x;
    // // f_pre_pose[1] = F_cur_y;
