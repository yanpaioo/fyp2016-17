...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  for (int i = 1; i < gnum_readings; ++i)
  {
    ...
    //if (laserscan.ranges[i] > grange) gpre_val = 0;

    if (abs(gpre_val - laserscan.ranges[i]) < 0.3) 
    {
    ...
    else
    {
      ...
      while(abs(gpre_val - laserscan.ranges[i+count+1]) < 0.2)
      {

        if (gnum_readings == i+count+1) goto checking;
        gpre_val = laserscan.ranges[i+count+1];
        count++;
      }
      checking:
      ...
      {
        position[num][0] = i;
        position[num][1] = laserscan.ranges[i];
        num++;
      }
      i = i + count;
      gpre_val = 0;
