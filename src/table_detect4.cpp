...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  for (int i = 1; i < i_no_readings; ++i)
  {
    ...
    if (i_pt_ct == 1) f_clus_sum = Msg_laser.ranges[i];

    else
    {
      for (int y = -1; y < (i_pt_ct-1); ++y)
      {
    ...
    if (i_pt_ct == 1)
    {
      f_clus_centre[i_clus_ct][0] = ave * sin((i) * Msg_laser.angle_increment);
      f_clus_centre[i_clus_ct][1] = (-1) * ave * cos((i) * Msg_laser.angle_increment);
    }
    else
    {
      f_clus_centre[i_clus_ct][0] = ave * sin((i+((i_pt_ct+1)/2)) * Msg_laser.angle_increment);
      f_clus_centre[i_clus_ct][1] = (-1) * ave * cos((i+((i_pt_ct+1)/2)) * Msg_laser.angle_increment);
    }
