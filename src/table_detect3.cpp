...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  for (int i = 1; i < i_no_readings; ++i)
  {
    ...
    {
      f_clus_centre[i_clus_ct][0] = ave * sin((i+((float)i_pt_ct/2)) * Msg_laser.angle_increment);
      f_clus_centre[i_clus_ct][1] = (-1) * ave * cos((i+(i_clus_ct/2)) * Msg_laser.angle_increment);
    }
