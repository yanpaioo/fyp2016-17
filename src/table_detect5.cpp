...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  for (int i = 1; i < i_no_readings; ++i)
  {
    ...
    if (i_pt_ct == 1)
    {
      f_clus_centre[i_clus_ct][0] = Msg_laser.ranges[i] * sin((i) * Msg_laser.angle_increment);
      f_clus_centre[i_clus_ct][1] = (-1) * Msg_laser.ranges[i] * cos((i) * Msg_laser.angle_increment);
    }
    else
    {
      for (int y = 0; y < i_pt_ct; ++y)
      {
        ...
        //ROS_INFO("y - %d   i_pt_ct - %d", y, i_pt_ct);
      }
      float ave;
      ave = f_clus_sum / i_pt_ct;

      f_clus_centre[i_clus_ct-1][0] = ave * sin((i+((i_pt_ct)/2)) * Msg_laser.angle_increment);
      f_clus_centre[i_clus_ct-1][0] = (-1) * ave * cos((i+((i_pt_ct)/2)) * Msg_laser.angle_increment);
    }
    i_clus_ct++;
    i += i_pt_ct;
    i_pt_ct = 1;
