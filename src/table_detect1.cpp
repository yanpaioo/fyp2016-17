...
void go(int offset, int k)
{
  ...
  if ( k == 0)
  {
    ...
    //ROS_INFO("position[0][0], position[0][1] = %f %f", position[0][0], position[0][1]);
    //ROS_INFO("position[1][0], position[1][1] = %f %f", position[1][0], position[1][1]); 
    //ROS_INFO("position[2][0], position[2][1] = %f %f", position[2][0], position[2][1]);
    ...
    //ROS_INFO("x[0], y[0], x[1], y[1]), x[2], y[2] = %f, %f,%f,%f,%f,%f",x[0], y[0], x[1], y[1], x[2], y[2]);
...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  for (int i = 1; i < gnum_readings; ++i)
  {
    ...
    if (gdiff >= 0.20)
    {
      ...
      if ((count < 30) && (laserscan.ranges[i] <= 2.5))
      {
        position[num][0] = i;
