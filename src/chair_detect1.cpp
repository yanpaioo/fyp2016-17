...
float F_chair_pose[2][2];       // {center, 2 pts making length, 2 pts making width}
...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
  ...
  for (int i = 0; i < i_comb_ct; i++)
  {
    ...
    ROS_INFO("Dist = %f", f_dist);

    if ((f_dist > F_chair_dim[0]-F_chair_dim_thres) && (f_dist < F_chair_dim[0]+F_chair_dim_thres))
    {
      int i_pt_index[4];
      ...
      chair_pose.position.x = p.x = p.x / 2.0;
      chair_pose.position.y = p.y = p.y / 2.0;


      chair.points.push_back(p);

      float x1, x2, y1, y2;
      x1 = F_clus_centre[V_combResult[i][0]][0];
      y1 = F_clus_centre[V_combResult[i][0]][1];

      x2 = F_clus_centre[V_combResult[i][1]][0];
      y2 = F_clus_centre[V_combResult[i][1]][1];
      ...
      chair_pose.position.z = acos((b*b + c*c - a*a)/(2*b*c)) - (3.1416/2);
      //chair_pose.position.z = -acos((b*b + c*c - a*a)/(2*b*c)) + 3.1416/2;
      if (x2 < x1)
      {
        chair_pose.position.z = -chair_pose.position.z;
      } 
